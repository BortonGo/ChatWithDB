    ...
    while (true) {
        ...
        while (true) {
            send_message(client_socket,
                "\n=== Меню пользователя ===\n"
                "1. Отправить сообщение (по логину и тексту)\n"
                "2. Входящие\n"
                "3. Выйти из аккаунта\n"
            ...
            if (input == "1") {
                std::string target = recv_line(client_socket);
                std::string message = recv_line(client_socket);

                if (target.empty() || message.empty()) {
                    send_message(client_socket, "Ошибка: не указан логин или сообщение.\n");
                }
                else {
                    ...
                    if (online_users.count(target)) {
                        ...
                        logger.write("Message from " + username + " to " + target + ": " + message);
                ...
                send_message(client_socket, "MSG_OK\n");
            }
            else if (input == "2") {
                ...
                logger.write("User logged out: " + username);
                ...
                std::cout << "[DEBUG] Клиент отключился до входа\n";
                logger.write("Client disconnected before login");
                closesocket(client_socket);
                return;
            }
            else {
